...
export default function App() {
  ...
  // Playlist of hero background videos (Azul drone videos uploaded by user: azul01.mp4 and azul02.mp4)
  const heroVideoPlaylist = useMemo(() => [
    getAssetUrl('/azul01.mp4'),
    getAssetUrl('/azul02.mp4'),
  ], []);
  const [currentVideoIndex, setCurrentVideoIndex] = useState(0);

  // Play video reliably on mount / index change
  useEffect(() => {
    ...
    if (playPromise !== undefined) {
      playPromise.catch(() => {
        // Autoplay policy fallback: play on first user gesture
      });
    }
  ...
  // Light cinematic film filter tuned for high legibility, soft warm tones, and bright airy clarity
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-800 flex flex-col font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div id="hero" className={`relative z-10 bg-[#121212] text-white overflow-hidden min-h-[100dvh] flex flex-col justify-between ${isAdminLoggedIn ? 'pt-[52px]' : ''}`}>
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 bg-slate-950">
          ...
          <video
            ref={heroVideoRef}
            key={heroVideoPlaylist[currentVideoIndex]}
            src={heroVideoPlaylist[currentVideoIndex]}
            autoPlay
            muted
            playsInline
            preload="auto"
        ...
        <section id="propiedades" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
          ...
          {(filters.propertyType !== 'TODOS' ||
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2">
              ...
              {filters.onlyRecentlyUploaded && (
                <span className="text-xs bg-amber-50 text-amber-950 font-bold px-2.5 py-1 rounded-md border border-amber-200">
                  🔥 Recién Subidas
                </span>
              )}
              {filters.propertyType !== 'TODOS' && (
