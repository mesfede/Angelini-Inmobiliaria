...
export const formatLocationName = (zone?: string, city?: string): string => {
  const cleanCity = (city || 'Azul').trim();
...
export const formatFullAddress = (address?: string, zone?: string, city?: string): string => {
  ...
  // Avoid duplicating if the address already starts with or contains the location name
  if (cleanAddr.toLowerCase().startsWith(locName.toLowerCase())) {
    return cleanAddr;
  }
