      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="bg-gradient-to-r from-[#071D3F] via-[#0B2F64] to-[#051329] text-white p-5 flex items-center justify-between border-b border-white/15">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-[#D3122A] rounded-xl text-white shadow-sm">
              <Calculator className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-xl font-bold font-['Playfair_Display','Libre_Baskerville',Georgia,serif]">Tasaciones</h2>
              <p className="text-xs text-slate-300 font-medium">Angelini Inmobiliaria · Azul, Bs. As.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <p className="text-xs text-slate-600">
              ...
              {/* Tipo de Inmueble */}
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">Tipo de Inmueble</label>
                <select
                  value={formData.propertyType}
                  onChange={(e) =>
                    setFormData({ ...formData, propertyType: e.target.value as PropertyType })
                  }
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 text-xs font-semibold focus:ring-2 focus:ring-[#0B2F64]"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">Dirección o Referencia</label>
                  <input
                    type="text"
                    placeholder="Ej: De Paula 1216"
                    value={formData.address}
                    onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 text-xs focus:ring-2 focus:ring-[#0B2F64]"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">Zona / Localidad</label>
                  <input
                    type="text"
              ...
              <div className="pt-2 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">
                    Nombre Completo <span className="text-[#D3122A]">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">
                    Teléfono de Contacto <span className="text-[#D3122A]">*</span>
                  </label>
                  <input
                    type="tel"
                    required
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">Observaciones adicionales</label>
                <textarea
                  rows={2}
                  placeholder="Detalles sobre el estado del inmueble o consulta..."
                  value={formData.comments}
                  onChange={(e) => setFormData({ ...formData, comments: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2 text-xs focus:ring-2 focus:ring-[#0B2F64]"
                />
              </div>

              <button
                type="submit"
                className="w-full bg-[#D3122A] hover:bg-[#B30E22] text-white py-3.5 rounded-xl font-bold text-sm shadow-md transition-all cursor-pointer"
              >
                Solicitar Tasación
              </button>
            </form>
